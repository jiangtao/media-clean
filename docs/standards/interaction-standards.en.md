...
17. The swipe scope should default to the current image list, meaning the filtered issue list after the user enters a category. The completed scan summary must not stack a second image grid underneath it. If that list contains only `1` media item, detail stays in single-item review.
...
22. If the media belongs to a duplicate group, the detail view should still make that relationship clear through labels and badges, but horizontal swiping should continue through the current result list. The user can act directly on the focused item with `Clear` or `Keep` instead of entering an extra complex subflow.
...
27. If detail contains only `1` media item, it must not show previous / next controls. When the current result list contains multiple items, the previous control must disappear at the first item and the next control must disappear at the last item. Wrap-around navigation is not allowed.
