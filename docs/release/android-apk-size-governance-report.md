...
补充验证：2026-05-14，使用 release-like `.debug` validation 包验证 23.690 MiB 候选。产物为 `android/app/build/outputs/apk/validation/app-validation.apk`，包名 `com.jt.mistapmediacleaner.debug`，`versionName=0.0.4-validation`，`versionCode=4`。首次普通 `adb install -r` 仍被 MIUI 返回 `INSTALL_FAILED_USER_RESTRICTED`；确认设备上旧 `.debug` 包只存在于 `user 10` 后，先执行 `cmd package install-existing --user 0 com.jt.mistapmediacleaner.debug`，再重新 `adb install -r` 覆盖安装成功。启动 `MainActivity` 后前台窗口为 `com.jt.mistapmediacleaner.debug/com.jt.mistapmediacleaner.MainActivity`，近端 logcat 未出现 `FATAL EXCEPTION`、`AndroidRuntime`、`UnsatisfiedLinkError` 或 `SoLoader` 崩溃。该结果证明 legacy packaging + shrink 的安装与启动 smoke 可行；`0.0.5` 正式 release 默认启用该组合，仍保留 workflow 手动关闭开关用于回滚定位。

0.0.5 release smoke：2026-05-14，本地临时签名 release 按线上默认策略重新构建，`versionName=0.0.5`，`versionCode=5`，SHA256 `eae36d4c1e22833dc797787620b43b9b5cd964e721668ac1cda6a59e52137419`。体积 gate 结果为 24,840,624 bytes / 23.690 MiB，ABI 为 `arm64-v8a,armeabi-v7a`，native lib 12.851 MiB，dex 4.616 MiB，JS bundle 2.811 MiB，无 warning / failure。正式线上签名包仍以 GitHub Actions release workflow 产出的 checksum、签名报告和 page 下载验证为准。
...
| legacy packaging | APK | 30.229 | -21.600 | -67.535 | `arm64-v8a`, `armeabi-v7a` | 12.851 | 10.905 | 2.811 | 已构建和过 size gate；组合 validation 包安装 / 启动通过 |
| legacy packaging + shrink | APK | 23.690 | -28.139 | -74.074 | `arm64-v8a`, `armeabi-v7a` | 12.851 | 4.616 | 2.811 | `0.0.5` 正式 release 默认策略；待线上正式签名链复核 |
...
5. 23.690 MiB 候选包通过 `.debug` validation 路径完成当前用户安装和启动 smoke；首次 MIUI 拦截属于设备侧 ADB 安装限制，不是 APK 打包失败。因此 `0.0.5` 正式 release 默认启用 legacy packaging + R8 / resource shrink，同时保留 workflow 手动关闭开关用于回滚定位。
...
| Stage 5 | P1 | 验证 native `.so` packaging / legacy packaging | 0.0.4 已实测；`.debug` validation 组合包当前用户安装 / 启动通过，`0.0.5` 正式 release 默认开启 | 单独开启 30.229 MiB；与 shrink 叠加 23.690 MiB | 安装、启动时间、低端机、签名、page 下载全部通过 |
| Stage 6 | P2 | 验证 R8 / resource shrink | 0.0.4 已实测；`.debug` validation 组合包当前用户安装 / 启动通过，`0.0.5` 正式 release 默认开启 | 单独开启 45.283 MiB；与 legacy packaging 叠加 23.690 MiB | media permission、notifications、foreground service、SQLite、image/video preview、native scan 全覆盖 |
...
1. `0.0.5` 正式 release 默认采用双 ARM ABI + R8/resource shrink + legacy native packaging，目标 page 单 APK 约 23.690 MiB；CI 正式 release 后继续复核正式签名链和 page 下载链路。
2. 若产品确认可以放弃 32-bit ARM，下一版可切 Stage 4/7：`arm64-v8a` 单 ABI；0.0.4 当前估算 `arm64 + legacy + shrink` 约 17.707 MiB，需要重新构建确认。
3. legacy packaging 和 R8/resource shrink 已进入默认策略；若线上或真机核心流程出现回归，优先通过 workflow 手动开关逐项关闭定位。
4. 发布前后继续覆盖权限、通知、后台扫描、SQLite、图片 / 视频预览和 native scan 回归。
